...
async function createProject(req, res) {
  const { creator, title, description, difficulty, funded, amount, start_time, end_time } = req.body; 
  const db = req.app.get('db');

  try {
    const projects = await db.project.create(creator, title, description, difficulty, funded, amount, start_time, end_time);
